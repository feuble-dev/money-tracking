# -*- coding: utf-8 -*-
"""
Peuple le catalogue (Pays/Operateurs/Types/Patterns) a partir de vrais SMS
Orange Money / Moov Money / Coris Money / Wave. Idempotent (get_or_create /
filter().exists() partout) : peut etre relance autant de fois que necessaire
sans creer de doublons, notamment pour ajouter les logos apres coup (voir
attach_logo_if_available).

Usage : python manage.py seed_catalog

Remplace l'ancien `_seed_catalog.py` (execute via `python manage.py shell <
_seed_catalog.py`) : piper un script multi-fonctions dans une console
interactive est fragile (le decoupage en blocs par l'interpreteur peut se
desynchroniser selon les lignes vides), une vraie management command evite
ce probleme structurellement.

Logos : cette command ne fabrique ni ne telecharge aucun logo (ce seraient
des marques deposees, pas des assets qu'on peut generer/emprunter). Depose
les vrais fichiers dans backend/_seed_logos/<slug>.png|jpg|jpeg|webp (slug =
nom en minuscules, espaces -> underscores, ex: orange_money.png) puis
relance cette meme command : elle ne re-uploade que les operateurs sans
logo existant. A defaut, uploade-les un par un depuis le dashboard admin
(/catalog/operators -> Modifier -> Logo).
"""
import os
...
from catalog.models import (
    Country, Operator, TransactionType, OperatorTransactionType, SmsPattern,
)

LOGOS_DIR = os.path.join(settings.BASE_DIR, '_seed_logos')
...
def attach_logo_if_available(operator, name, stdout):
    if operator.logo:
        return
    ...
    for ext in LOGO_EXTENSIONS:
        ...
        if os.path.isfile(path):
            with open(path, 'rb') as f:
                operator.logo.save(slug + ext, File(f), save=True)
            stdout.write(f'Logo attache : {name} <- {path}')
            return


class Command(BaseCommand):
    help = "Peuple le catalogue avec Burkina Faso + 4 operateurs reels + leurs patterns SMS."
    ...
    def handle(self, *args, **options):
        ...
        def add_pattern(operator_name, type_code, raw, fields, direction_override=None):
            ...
            existing = SmsPattern.objects.filter(operator_transaction_type=link, raw_example=raw).first()
            if existing is None:
                ...
                return
            # Upsert : si les zones taguees ont ete corrigees depuis le dernier
            # seed (ex: un champ date oublie qui figeait la date en texte
            # litteral dans la regex mobile), on met a jour plutot que de
            # rejouer silencieusement l'ancienne version buguee.
            if existing.tagged_zones != zones or existing.direction_override != direction_override:
                existing.tagged_zones = zones
                existing.direction_override = direction_override
                existing.save()
        ...
        add_pattern('Coris Money', 'depot',
            ...
            [('nom_client', 'LENGANE'), ('montant', '2500.0'),
             ('date', '28/04/2026 17:51:04'),
             ('operator_transaction_id', '20264284.US0001291.972')])
        ...
        add_pattern('Coris Money', 'paiement_marchand',
            ...
            [('montant', '538.0'),
             ('date', '06/12/2025 08:57:53'),
             ('numero_client', '0600603'), ('nom_client', 'KREEZUS'),
        ...
        self.stdout.write(self.style.SUCCESS('=== Catalogue peuple ==='))
        for op_name, op in operator_objs.items():
            ...
            logo_state = 'avec logo' if op.logo else 'SANS logo'
            self.stdout.write(f"{op_name}: {n_types} type(s) actives, {n_patterns} pattern(s), {logo_state}")
