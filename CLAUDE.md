...
mobile/
  lib/
    core/
      ...
      sms/             # SmsListenerService (EventChannel), SmsMatchingEngine (unified), SmsFieldExtractor (legacy fallback), SmsPatternBuilder (real regex compiler), CommissionCalculator, SmsCatchupService (D15, missed-SMS catch-up on start)
...
### Database
- SQLite via `sqflite`, singleton in `DatabaseHelper.instance`, **DB version 11** (v10: `v_transactions_with_operator` gained a `transaction_types` join for `type_label`, plus a one-time `refreshAllSummaries()` — see "Transaction display/creation" section below; v11: `caisse.solde_ref_at` for the announced-balance tracking described below)
...
- Extract fields, dedup by `operator_transaction_id`, create transaction with commission (`CommissionCalculator`, single implementation shared by all three entry points and `TransactionTypeModel`)
- A detected transaction is created **directly `status: 'completed'`** — no manual confirmation step. `'pending'` is not written anywhere in the app anymore (manual creation already defaulted to `'completed'`, history import already used `'completed'`; only the real-time/background SMS path used to write `'pending'`). The detection notification (`NotificationService.showPendingTransactionNotification`, name kept for now) only offers "Annuler" (undo a wrongly-detected transaction), not "Confirmer". The old pending-confirmation screens/routes are left in place (harmless, no longer fed) in case a device still has legacy `'pending'` rows from before this change.
...
- **Transaction creation UI** (`NewTransactionScreen`, route `/transactions/new`, no more `/transactions/new/:type`) no longer has separate "dépôt"/"retrait" FAB buttons or a route-level type param. Flow is now: pick operator → pick from *that operator's* active `operator_transaction_types` (label, direction, USSD, commission all come from the link, D3) → fill client/amount → dial USSD or create manually. `SmsWaitingScreen` (dead code, relied on the now-removed `'pending'` status — see below) was deleted.
...
- **SMS-detected transactions no longer require manual confirmation** — `sms_processing_pipeline.dart` creates them directly `status: 'completed'` (matching manual and history-import creation, which already did). The detection notification only offers "Annuler" now, not "Confirmer". `'pending'` is not written anywhere in the app anymore; the pending-confirmation screens/routes are left in place (harmless) in case a device still has legacy `'pending'` rows from before this change.
...
- Android permissions required: `RECEIVE_SMS`, `READ_SMS`, `CALL_PHONE`, `USE_BIOMETRIC`, `POST_NOTIFICATIONS`
